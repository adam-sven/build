...
export async function POST(request: NextRequest) {
  ...
  const challenge = await consumeAndVerifyChallenge({ kind: "submit", nonce, wallet, mint, chain, message, signatureBase64 });
  if (!challenge.ok) return err(challenge.code, challenge.code === "nonce_expired" ? "Challenge expired" : "Signature verification failed", 400);

  if (await hasSubmissionForMint(chain, mint)) {
    return err("token_already_submitted", "This token is already listed.", 409);
  }

  if (await hasFeeSigUsed("submit", feeTxSig)) return err("submit_fee_reused", "Submit fee signature already used.", 409);

  const feeCheck = await verifySystemTransferFee({ expectedFrom: wallet, feeTxSig, lamports: SUBMIT_FEE_LAMPORTS });
  if (!feeCheck.ok) return err("submit_fee_invalid", "Submit fee tx must transfer exactly 0.01 SOL to treasury.", 400);
