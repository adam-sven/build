...
export async function POST(request: NextRequest) {
  ...
  if (ageDays === null || ageDays < 7) {
    return err("ineligible_wallet_age", "Wallet must be active for at least 7 days.", 403);
  }
  if (balance === null || balance < 0.1) {
    return err("ineligible_sol_balance", "Wallet balance must be at least 0.1 SOL.", 403);
  }

  const lastVote = await getVoteCooldown(chain, mint, wallet);
  if (lastVote && Date.now() - lastVote.getTime() < 3600_000) {
    return err("already_voted_cooldown", "You can vote once per wallet/mint per hour.", 409);
  }

  const dailyCount = await getVoteDailyCount(chain, wallet);
  if (dailyCount >= 25) {
    return err("daily_vote_limit", "Daily vote limit reached (25).", 429);
  }

  if (await hasFeeSigUsed("vote", feeTxSig)) {
    return err("fee_tx_reused", "Fee transaction signature already used.", 409);
  }
  ...
  if (!feeCheck.ok) {
    return err("invalid_fee_tx", "Vote fee transaction must transfer exactly 0.001 SOL to treasury.", 400);
  }
