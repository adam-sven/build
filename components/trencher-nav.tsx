    ...
    <>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm md:hidden" onClick={() => setMobileOpen(false)}>
          ...
          >
            <div className="mb-4 flex items-center justify-between">
              <Link href="/" className="flex items-center gap-2 text-sm font-semibold text-white" onClick={() => setMobileOpen(false)}>
      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-56 border-r border-white/10 bg-[#090d14] md:block">
        <div className="flex h-full flex-col p-3">
          <Link href="/" className="mb-4 flex items-center gap-2 rounded-md px-2 py-2 text-sm font-semibold text-white">
            <Image src="/trencher-mark.svg" alt="iamtrader.fun" width={18} height={18} className="h-[18px] w-[18px]" />
            iamtrader.fun
          </Link>
          <nav className="space-y-1">
